/**
 *	DES reduit a sept tours, sans permutation initiale.
 *
 */
...
//Chiffrement DES reduit a sept tours
...
void des_encryption(ROUND_BLOCK exp_key[16], BLOCK plaintext) {
    ...
    DES_Final_Round(plaintext, exp_key[6]);
}
...
void des_decryption(ROUND_BLOCK exp_key[16], BLOCK ciphertext) {
	...
	for(i=0; i<6; i++) {
		DES_Round(ciphertext, exp_key[6 - i]);
	}
