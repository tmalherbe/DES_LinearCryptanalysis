/**
 *	DES reduit a cinq tours, sans permutation initiale.
 *
 */
...
//Chiffrement DES reduit a cinq tours
...
void des_encryption(ROUND_BLOCK exp_key[16], BLOCK plaintext) {
    ...
    DES_Final_Round(plaintext, exp_key[4]);
}
...
void des_decryption(ROUND_BLOCK exp_key[16], BLOCK ciphertext) {
	...
	for(i=0; i<4; i++) {
		DES_Round(ciphertext, exp_key[4 - i]);
	}
