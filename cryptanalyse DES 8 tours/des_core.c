/**
 *	DES reduit a huit tours, sans permutation initiale.
 *
 */
...
//Chiffrement DES reduit a huit tours
...
void des_encryption(ROUND_BLOCK exp_key[16], BLOCK plaintext) {
    ...
    DES_Final_Round(plaintext, exp_key[7]);
}
...
void des_decryption(ROUND_BLOCK exp_key[16], BLOCK ciphertext) {
	...
	for(i=0; i<7; i++) {
		DES_Round(ciphertext, exp_key[7 - i]);
	}
